...
const TEAM_NAMES = ["Echipa A (Verde)", "Echipa B (Portocaliu)", "Echipa C (Negru)"];
...
export default function FootballMatchesPanel({ groupId, eventId, stageNumber, teams, canManage }: { groupId: string; eventId: string; stageNumber: number; teams?: GeneratedTeams | null; canManage: boolean }) {
  ...
  return <div className="flex flex-col gap-4">
    ...
    {(showForm || editing) && <MatchEditor groupId={groupId} eventId={eventId} stageNumber={stageNumber} teams={availableTeams} cards={cards} initial={editing} nextOrder={nextOrder} saving={saving} onCancel={() => { setShowForm(false); setEditing(null); }} onSave={persist} />}
    <MatchList matches={matches} canManage={canManage} onEdit={(match) => { setEditing(match); setShowForm(false); }} />
