...
function MatchEditor({ groupId,eventId,stageNumber,teams,teamLabels,cards,initial,nextOrder,saving,onCancel,onSave }: { groupId:string;eventId:string;stageNumber:number;teams:ParticipantEntry[][];teamLabels:string[];cards:PlayerCardData[];initial:FootballMatch|null;nextOrder:number;saving:boolean;onCancel:()=>void;onSave:(match:FootballMatch)=>void }) {
  ...
  return <section className="event-panel p-5 sm:p-6"><h2 className="event-panel-title">{initial ? "Editează meciul" : "Meci nou"}</h2>
    ...
    <div className="mt-5 grid gap-4 md:grid-cols-2">{[0,1].map((teamIndex) => <div key={teamIndex} className="rounded-xl border border-border p-4"><div className="flex items-center justify-between gap-3"><h3 className="font-bold">{names[teamIndex]}</h3><div className="text-xs font-semibold"><span className="block">Autogoluri în favoare</span><GoalStepper value={ownGoals[teamIndex] ?? 0} label={`Autogoluri în favoarea ${names[teamIndex]}`} onChange={(value) => { setOwnGoals((current) => current.map((item, index) => index === teamIndex ? value : item)); setPenalty(null); }} /></div></div><div className="mt-3 flex flex-col gap-2">{players.filter((player) => player.teamIndex === teamIndex).map((player) => <div key={player.userId} className="grid grid-cols-[minmax(0,1fr)_76px_auto] items-center gap-2"><span className="truncate text-sm font-medium">{player.name}</span><select aria-label={`Poziția lui ${player.name}`} value={player.position} onChange={(event) => setPlayers((current)=>current.map((item)=>item.userId===player.userId?{...item,position:event.target.value as MatchPosition}:item))} className="rounded-lg border border-border bg-background px-2 py-2 text-xs">{POSITIONS.map((position)=><option key={position}>{position}</option>)}</select><GoalStepper value={player.goals} label={`Goluri ${player.name}`} onChange={(goals) => { setPlayers((current) => current.map((item) => item.userId === player.userId ? { ...item, goals } : item)); setPenalty(null); }} /></div>)}</div></div>)}</div>
...
function GoalStepper({ value, label, onChange }: { value: number; label: string; onChange: (value: number) => void }) {
  return <div className="mt-1.5 inline-flex items-center overflow-hidden rounded-lg border border-border bg-background" aria-label={label}>
    <button type="button" aria-label={`Scade ${label}`} onClick={() => onChange(Math.max(0, value - 1))} disabled={value === 0} className="flex h-9 w-9 items-center justify-center text-lg font-bold text-foreground transition hover:bg-muted disabled:cursor-not-allowed disabled:opacity-40">−</button>
    <span className="flex h-9 min-w-9 items-center justify-center border-x border-border px-2 text-base font-extrabold text-foreground" aria-live="polite">{value}</span>
    <button type="button" aria-label={`Adaugă ${label}`} onClick={() => onChange(value + 1)} className="flex h-9 w-9 items-center justify-center text-lg font-bold text-primary transition hover:bg-primary/10">+</button>
  </div>;
}
