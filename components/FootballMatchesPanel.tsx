...
function MatchEditor({ groupId,eventId,stageNumber,teams,initial,nextOrder,saving,onCancel,onSave }: {groupId:string;eventId:string;stageNumber:number;teams:ParticipantEntry[][];initial:FootballMatch|null;nextOrder:number;saving:boolean;onCancel:()=>void;onSave:(match:FootballMatch)=>void}) {
  ...
  const winnerIndexes = scores.map((score, index) => score === maxScore ? index : -1).filter((index) => index >= 0);
  const tied = winnerIndexes.length > 1;
  const selectedPenalty = tied && penalty !== null && winnerIndexes.includes(penalty) ? penalty : null;
  return <section className="event-panel p-5 sm:p-6"><h2 className="event-panel-title">{initial ? "Editează meciul" : "Meci nou"}</h2><div className="mt-4 flex flex-wrap gap-3">{teams.map((_,i)=><label key={i} className="flex items-center gap-2"><span className="text-sm font-semibold">{names[i]}</span><input type="number" min={0} value={scores[i] ?? 0} onChange={(e)=>{setScores(v=>v.map((x,j)=>j===i?Number(e.target.value):x));setPenalty(null);}} className="w-20 rounded-lg border border-border bg-background px-3 py-2" /></label>)}<label className="flex items-center gap-2"><span className="text-sm">Ordine</span><input type="number" min={1} value={order} onChange={(e)=>setOrder(Number(e.target.value))} className="w-20 rounded-lg border border-border bg-background px-3 py-2" /></label></div>{tied && <fieldset className="mt-4 rounded-xl border border-primary/40 bg-primary/5 p-4"><legend className="px-1 text-sm font-bold text-foreground">Cine a câștigat la penalty?</legend><div className="mt-2 flex flex-wrap gap-2">{winnerIndexes.map((index)=><button key={index} type="button" aria-pressed={selectedPenalty===index} onClick={()=>setPenalty(index)} className={`rounded-xl border px-4 py-2.5 text-sm font-semibold ${selectedPenalty===index ? "border-primary bg-primary text-primary-foreground" : "border-border bg-background text-foreground"}`}>{names[index]}</button>)}</div>{selectedPenalty===null && <p className="mt-2 text-sm font-medium text-destructive">Alegerea câștigătoarei este obligatorie.</p>}</fieldset>}<div className="mt-5 grid gap-4 md:grid-cols-2">{teams.map((team,teamIndex)=><div key={teamIndex} className="rounded-xl border border-border p-3"><h3 className="font-bold">{names[teamIndex]}</h3><div className="mt-3 flex flex-col gap-2">{team.map((participant)=>{const p=players.find(x=>x.userId===participant.userId)!;return <div key={participant.userId} className="grid grid-cols-[1fr_72px_64px] items-center gap-2"><span className="truncate text-sm font-medium">{participant.name}</span><select value={p?.position ?? "MID"} onChange={(e)=>setPlayers(v=>v.map(x=>x.userId===participant.userId?{...x,position:e.target.value as MatchPosition}:x))} className="rounded-lg border border-border bg-background px-2 py-2 text-xs">{POSITIONS.map(pos=><option key={pos}>{pos}</option>)}</select><input aria-label={`Goluri ${participant.name}`} type="number" min={0} value={p?.goals ?? 0} onChange={(e)=>setPlayers(v=>v.map(x=>x.userId===participant.userId?{...x,goals:Number(e.target.value)}:x))} className="rounded-lg border border-border bg-background px-2 py-2 text-sm" /></div>})}</div></div>)}</div><div className="mt-5 flex gap-2"><button disabled={saving || teams.length<2 || (tied&&penalty===null)} type="button" onClick={()=>onSave({id:initial?.id??"",groupId,eventId,stageNumber,matchOrder:order,playedAt:Date.now(),teamNames:names,scores,penaltyWinnerIndex:tied?penalty:null,players})} className="rounded-xl bg-primary px-4 py-2.5 font-semibold text-primary-foreground disabled:opacity-50">Salvează și recalculează</button><button type="button" onClick={onCancel} className="rounded-xl border border-border px-4 py-2.5 font-semibold">Renunță</button></div></section>;
}
