...
export default function PlayerCard({ card, compact = false }: PlayerCardProps) {
  ...
  return (
    <article
      className={`relative overflow-hidden rounded-[26px] border border-white/60 bg-gradient-to-br ${tierClasses(tier)} shadow-xl ${compact ? "w-44" : "w-full max-w-xs"}`}
    >
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="mt-1 text-[10px] font-extrabold uppercase tracking-widest opacity-80">
              {isStage ? `Etapa ${card.stageNumber}` : tier}
            </div>
          </div>
        ...
        <div className="mt-3 text-center">
          ...
          {isStage && awards.length > 0 && (
            <div className="mt-2 flex flex-wrap justify-center gap-1">
              {awards.map((award) => (
                <span key={award.awardId} className="rounded-full bg-black/10 px-2 py-1 text-[10px] font-bold">
                  {award.label}
                </span>
              ))}
            </div>
