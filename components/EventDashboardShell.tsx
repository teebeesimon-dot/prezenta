...
export default function EventDashboardShell({ event, active = "overview", children }: { event: Event; active?: "overview" | "group" | "confirmed" | "teams" | "matches" | "standings" | "cards"; children: React.ReactNode }) {
  const links = [{ id: "overview", label: "Eveniment", href: `/event/${event.id}` }, { id: "group", label: "Grup", href: `/event/${event.id}/group` }, ...(event.sport === "football" ? [{ id: "teams", label: "Echipe", href: `/event/${event.id}/teams` }, { id: "matches", label: "Meciuri", href: `/event/${event.id}/matches` }, { id: "standings", label: "Clasament", href: `/event/${event.id}/standings` }, { id: "cards", label: "Player Cards", href: `/event/${event.id}/cards` }] : [])];
  return (
