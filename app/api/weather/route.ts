  ...
  try {
    ...
    return NextResponse.json({
      ...
      hourly: data.hourly.time.slice(0, 24).map((time: string, index: number) => ({
        time, temperature: data.hourly.temperature_2m[index], code: data.hourly.weather_code[index],
        label: weatherLabels[data.hourly.weather_code[index]] ?? "Variabil", precipitation: data.hourly.precipitation_probability[index],
      })),
    });
  } catch {
